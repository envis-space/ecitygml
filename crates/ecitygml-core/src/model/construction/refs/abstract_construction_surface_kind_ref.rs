...
pub enum AbstractConstructionSurfaceKindRef<'a> {
...
impl<'a> From<&'a AbstractConstructionSurfaceKind> for AbstractConstructionSurfaceKindRef<'a> {
    fn from(item: &'a AbstractConstructionSurfaceKind) -> Self {
        match item {
...
impl<'a> AsAbstractConstructionSurface for AbstractConstructionSurfaceKindRef<'a> {
...
crate::impl_abstract_construction_surface_traits!(AbstractConstructionSurfaceKindRef<'_>);

impl<'a> HasFeatureType for AbstractConstructionSurfaceKindRef<'a> {
...
macro_rules! impl_from_construction_surface_kind_ref {
    ($type:ident) => {
        impl<'a> From<&'a $type>
            for $crate::model::construction::refs::AbstractConstructionSurfaceKindRef<'a>
        {
            fn from(x: &'a $type) -> Self {
                $crate::model::construction::refs::AbstractConstructionSurfaceKindRef::$type(
                    x.into(),
                )
            }
        }
        $crate::impl_from_for_thematic_surface_kind_ref!(AbstractConstructionSurfaceKind, $type);
    };
}
...
macro_rules! impl_try_from_construction_surface_kind_ref {
    ($type:ident) => {
        impl<'a> TryFrom<$crate::model::construction::refs::AbstractConstructionSurfaceKindRef<'a>>
            for &'a $type
        {
            type Error = ();
            fn try_from(
                x: $crate::model::construction::refs::AbstractConstructionSurfaceKindRef<'a>,
            ) -> Result<Self, ()> {
                match x {
                    $crate::model::construction::refs::AbstractConstructionSurfaceKindRef::$type(k) => {
        ...
        $crate::impl_try_from_for_thematic_surface_kind_ref!(AbstractConstructionSurfaceKind, $type);
    };
}
...
impl_try_from_thematic_surface_kind_ref_for_enum!(
    ...
    AbstractConstructionSurfaceKindRef
);
