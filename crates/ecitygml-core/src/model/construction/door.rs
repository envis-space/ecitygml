...
impl<'a> From<&'a Door> for FeatureRef<'a> {
    fn from(item: &'a Door) -> Self {
        FeatureRef::Door(item)
    }
}

impl<'a> From<&'a mut Door> for FeatureRefMut<'a> {
    fn from(item: &'a mut Door) -> Self {
        FeatureRefMut::Door(item)
    }
}
