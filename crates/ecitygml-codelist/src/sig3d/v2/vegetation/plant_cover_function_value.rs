...
/// The SIG3D standard code list `PlantCover_function.xml` (version 2.0).
///
/// <https://www.sig3d.de/codelists/standard/vegetation/2.0/PlantCover_function.xml>
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum Sig3dPlantCoverFunctionValue {
    Lemnetea,
...
impl CodeListValue for Sig3dPlantCoverFunctionValue {
    const CODE_SPACE: &'static str =
        "http://www.sig3d.org/codelists/citygml/2.0/vegetation/2.0/PlantCover_function.xml";
...
mod tests {
    use super::*;
    use ecitygml_core::model::vegetation::values::PlantCoverFunctionValue;
    use egml::model::basic_types::Code;

    #[test]
    fn round_trips_known_codes() {
        for (code, value) in [
            ("1010", Sig3dPlantCoverFunctionValue::Lemnetea),
            ("9999", Sig3dPlantCoverFunctionValue::Unknown),
        ] {
            assert_eq!(Sig3dPlantCoverFunctionValue::from_code_value(code), value);
    ...
    fn unrecognized_code_in_this_code_space_falls_back_to_other() {
        let value = Sig3dPlantCoverFunctionValue::from_code_value("999999");
        assert_eq!(
            value,
            Sig3dPlantCoverFunctionValue::Other("999999".to_string())
        );
    ...
    fn interprets_bare_code_with_no_code_space() {
        let wrapped = PlantCoverFunctionValue::from(Code::new("1010"));
        assert_eq!(
            wrapped.interpret::<Sig3dPlantCoverFunctionValue>(),
            Some(Sig3dPlantCoverFunctionValue::Lemnetea)
        );
    }

    #[test]
    fn does_not_interpret_a_different_code_space() {
        let wrapped = PlantCoverFunctionValue::from(Code::with_code_space(
            "https://example.org/local-codes",
            "1010",
        ));
        assert_eq!(wrapped.interpret::<Sig3dPlantCoverFunctionValue>(), None);
    }
}
