...
/// The SIG3D standard code list `PlantCover_class.xml` (version 2.0).
///
/// <https://www.sig3d.de/codelists/standard/vegetation/2.0/PlantCover_class.xml>
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum Sig3dPlantCoverClassValue {
    Lemnetea,
...
impl CodeListValue for Sig3dPlantCoverClassValue {
    const CODE_SPACE: &'static str =
        "http://www.sig3d.org/codelists/citygml/2.0/vegetation/2.0/PlantCover_class.xml";
...
mod tests {
    use super::*;
    use ecitygml_core::model::vegetation::values::PlantCoverClassValue;
    use egml::model::basic_types::Code;

    #[test]
    fn round_trips_known_codes() {
        for (code, value) in [
            ("1010", Sig3dPlantCoverClassValue::Lemnetea),
            ("9999", Sig3dPlantCoverClassValue::Unknown),
        ] {
            assert_eq!(Sig3dPlantCoverClassValue::from_code_value(code), value);
    ...
    fn unrecognized_code_in_this_code_space_falls_back_to_other() {
        let value = Sig3dPlantCoverClassValue::from_code_value("999999");
        assert_eq!(
            value,
            Sig3dPlantCoverClassValue::Other("999999".to_string())
        );
    ...
    fn interprets_bare_code_with_no_code_space() {
        let wrapped = PlantCoverClassValue::from(Code::new("1010"));
        assert_eq!(
            wrapped.interpret::<Sig3dPlantCoverClassValue>(),
            Some(Sig3dPlantCoverClassValue::Lemnetea)
        );
    }

    #[test]
    fn does_not_interpret_a_different_code_space() {
        let wrapped = PlantCoverClassValue::from(Code::with_code_space(
            "https://example.org/local-codes",
            "1010",
        ));
        assert_eq!(wrapped.interpret::<Sig3dPlantCoverClassValue>(), None);
    }
}
