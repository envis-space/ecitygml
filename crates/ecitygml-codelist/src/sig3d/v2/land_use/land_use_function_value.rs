...
/// The SIG3D standard code list `LandUse_function.xml` (version 2.0).
///
/// <https://www.sig3d.de/codelists/standard/landuse/2.0/LandUse_function.xml>
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum Sig3dLandUseFunctionValue {
    Residential,
    IndustryAndBusiness,
    MixedUse,
    SpecialFunctionArea,
...
impl CodeListValue for Sig3dLandUseFunctionValue {
    const CODE_SPACE: &'static str =
        "http://www.sig3d.org/codelists/citygml/2.0/landuse/2.0/LandUse_function.xml";
...
mod tests {
    use super::*;
    use ecitygml_core::model::land_use::values::LandUseFunctionValue;
    use egml::model::basic_types::Code;

    #[test]
    fn round_trips_known_codes() {
        for (code, value) in [
            ("1010", Sig3dLandUseFunctionValue::Residential),
            ("4040", Sig3dLandUseFunctionValue::Sea),
        ] {
            assert_eq!(Sig3dLandUseFunctionValue::from_code_value(code), value);
    ...
    fn unrecognized_code_in_this_code_space_falls_back_to_other() {
        let value = Sig3dLandUseFunctionValue::from_code_value("999999");
        assert_eq!(
            value,
            Sig3dLandUseFunctionValue::Other("999999".to_string())
        );
    ...
    fn interprets_bare_code_with_no_code_space() {
        let wrapped = LandUseFunctionValue::from(Code::new("1010"));
        assert_eq!(
            wrapped.interpret::<Sig3dLandUseFunctionValue>(),
            Some(Sig3dLandUseFunctionValue::Residential)
        );
    }

    #[test]
    fn does_not_interpret_a_different_code_space() {
        let wrapped = LandUseFunctionValue::from(Code::with_code_space(
            "https://example.org/local-codes",
            "1010",
        ));
        assert_eq!(wrapped.interpret::<Sig3dLandUseFunctionValue>(), None);
    }
}
