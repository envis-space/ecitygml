...
/// The SIG3D standard code list `CityFurniture_usage.xml` (version 2.0).
///
/// <https://www.sig3d.de/codelists/standard/cityfurniture/2.0/CityFurniture_usage.xml>
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum Sig3dCityFurnitureUsageValue {
...
impl CodeListValue for Sig3dCityFurnitureUsageValue {
    const CODE_SPACE: &'static str =
        "http://www.sig3d.org/codelists/citygml/2.0/cityfurniture/2.0/CityFurniture_usage.xml";
...
mod tests {
    use super::*;
    use ecitygml_core::model::city_furniture::values::CityFurnitureUsageValue;
    use egml::model::basic_types::Code;

    #[test]
    fn round_trips_known_codes() {
        for (code, value) in [
            ("1000", Sig3dCityFurnitureUsageValue::CommunicationFixture),
            ("1540", Sig3dCityFurnitureUsageValue::Others),
        ] {
            assert_eq!(Sig3dCityFurnitureUsageValue::from_code_value(code), value);
    ...
    fn unrecognized_code_in_this_code_space_falls_back_to_other() {
        let value = Sig3dCityFurnitureUsageValue::from_code_value("999999");
        assert_eq!(
            value,
            Sig3dCityFurnitureUsageValue::Other("999999".to_string())
        );
    ...
    fn interprets_bare_code_with_no_code_space() {
        let wrapped = CityFurnitureUsageValue::from(Code::new("1000"));
        assert_eq!(
            wrapped.interpret::<Sig3dCityFurnitureUsageValue>(),
            Some(Sig3dCityFurnitureUsageValue::CommunicationFixture)
        );
    }

    #[test]
    fn does_not_interpret_a_different_code_space() {
        let wrapped = CityFurnitureUsageValue::from(Code::with_code_space(
            "https://example.org/local-codes",
            "1000",
        ));
        assert_eq!(wrapped.interpret::<Sig3dCityFurnitureUsageValue>(), None);
    }
}
