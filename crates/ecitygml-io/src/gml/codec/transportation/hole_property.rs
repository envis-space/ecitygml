use crate::Error;
use crate::gml::codec::transportation::{deserialize_hole, serialize_hole};
use crate::gml::util::{CityGmlElement, CombinedCityGmlElement};
use ecitygml_core::model::transportation::HoleProperty;
...
pub fn deserialize_hole_property(
    xml_document: &[u8],
    spans: &XmlElementSpans<CombinedCityGmlElement>,
) -> Result<HoleProperty, Error> {
    let parsed: GmlHoleProperty = de::from_reader(xml_document)?;

    let mut object = None;

    if let Some(span) = spans.first(CityGmlElement::Hole.into()) {
        object = Some(deserialize_hole(&xml_document[span.start..span.end])?);
    }
...
pub fn serialize_hole_property(
    hole_property: &HoleProperty,
    formatting: Formatting,
) -> Result<XmlNode, Error> {
    let mut parts = XmlNodeParts::empty();
    parts.attributes.extend(serialize_association_attributes(
        hole_property.association(),
    ));
    parts
        .attributes
        .extend(serialize_ownership_attributes(hole_property.ownership()));
    if let Some(object) = hole_property.object() {
        parts
            .content
            .push(XmlNodeContent::Child(serialize_hole(object, formatting)?));
    }
    Ok(XmlNode::new(CityGmlElement::HoleProperty.into(), parts))
}

#[derive(Debug, Serialize, Deserialize, PartialEq, Clone)]
pub struct GmlHoleProperty {
    #[serde(flatten)]
...
mod tests {
    ...
    fn test_deserialize_basic_href_marking_property() {
        ...
        let xml_document =
            b"<tran:hole xlink:href=\"#DEBY_LOD2_59772_4becb506-d53b-44ca-a483-e6a3d238b4c2\" />";

        let spans = extract_xml_element_spans(xml_document).expect("should work");
        let hole_property = deserialize_hole_property(xml_document, &spans).expect("should work");

        assert_eq!(
            hole_property.href().expect("should exist"),
            &HRef::from("#DEBY_LOD2_59772_4becb506-d53b-44ca-a483-e6a3d238b4c2")
        );
    }

    #[test]
    fn test_deserialize_basic_hole_property() {
        use egml::io::util::extract_xml_element_spans;
        let xml_document =
            b"<tran:hole>
            <tran:Hole gml:id=\"UUID_caee8e69-7e58-373d-8851-18733ef1bd09\">
              <gml:name>PotHole 1</gml:name>
              <lod0MultiCurve>
                <gml:MultiCurve>
                  <gml:curveMember>
                    <gml:LineString>
        ...
        assert!(hole_property.object().is_some());
    }
}
