...
use crate::gml::codec::core::appearance_property::{
    deserialize_appearance_property, serialize_appearance_member_property,
};
use crate::gml::codec::core::city_object_property::{
    deserialize_city_object_property, serialize_city_object_member_property,
};
use crate::gml::codec::core::{
    deserialize_abstract_feature_with_lifespan, serialize_abstract_feature_with_lifespan,
};
use crate::gml::util::xml_element::XmlElement;
use crate::gml::util::{collect_children, extract_xml_element_spans, get_namespace_pairs};
...
pub fn deserialize_city_model(xml_document: &[u8]) -> Result<CityModel, Error> {
    ...
    rayon::scope(|s| {
        ...
        s.spawn(|_| {
            appearance_members_result = Some(collect_children(
                xml_document,
                &spans,
                XmlElement::AppearanceMemberProperty,
    ...
    let mut city_model = CityModel::new(abstract_feature_with_lifespan);
...
pub fn serialize_city_model<W: Write>(
    ...
    let elem = BytesStart::new(XmlElement::CityModel.as_str()).with_attributes(
        xml_node_parts
            .attributes
            .iter()
    ...
    let xml_nodes = city_model
        .city_object_members()
        .par_iter()
        .map(|x| serialize_city_object_member_property(x, formatting))
    ...
    let xml_nodes = city_model
        .appearance_members()
        .par_iter()
        .map(|x| serialize_appearance_member_property(x, formatting))
        .collect::<Result<Vec<_>, _>>()?;
    for current_xml_node in xml_nodes {
        current_xml_node.write_to_at_depth(xml_writer, formatting, 1)?;
    }

    xml_writer.write_event(Event::End(BytesEnd::new(XmlElement::CityModel.as_str())))?;
