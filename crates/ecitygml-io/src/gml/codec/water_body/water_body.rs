...
use ecitygml_core::model::water_body::values::{
...
pub fn deserialize_water_body(xml_document: &[u8]) -> Result<WaterBody, Error> {
    ...
    water_body.set_class_opt(parsed.class.map(Code::from).map(WaterBodyClassValue::from));
    water_body.set_functions(
        parsed
            .functions
            .into_iter()
            .map(Code::from)
            .map(WaterBodyFunctionValue::from)
            .collect(),
    );
    water_body.set_usages(
        parsed
            .usages
            .into_iter()
            .map(Code::from)
            .map(WaterBodyUsageValue::from)
            .collect(),
    );
...
impl From<&WaterBody> for GmlWaterBody {
    fn from(item: &WaterBody) -> Self {
        Self {
            class: item.class().map(WaterBodyClassValue::code).map(Into::into),
            functions: item
                .functions()
                .iter()
                .map(WaterBodyFunctionValue::code)
                .map(Into::into)
                .collect(),
            usages: item
                .usages()
                .iter()
                .map(WaterBodyUsageValue::code)
                .map(Into::into)
                .collect(),
        }
    }
