...
TEST_CASE("GeometryStore", "[reader]") {
...
TEST_CASE("GeometryStore", "[get]") {
    ...
    const auto city_object_geometry = city_model_geometry_store.get("GML_5856d7ad-5e34-498a-817b-9544bfbb1475");
...
TEST_CASE("GeometryStore", "[iterator]") {
    const GmlReader reader("./tests/fixtures/FZK-Haus-LoD2-KIT-IAI-KHH-B36-V1__v3.gml");
    auto city_model = reader.finish();
    const auto city_model_geometry_store = CityModelGeometryStore(&city_model);

    auto ids = city_model_geometry_store.get_object_ids();
